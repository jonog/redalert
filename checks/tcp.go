...
var NewTCP = func(config Config, logger *log.Logger) (Checker, error) {
	...
	return Checker(&TCP{tcpConfig.Address, tcpConfig.Port, logger}), nil
}
...
func (t *TCP) Check() (Metrics, error) {
	...
	t.log.Println("Establish TCP with", t.Address, "on port", t.Port)
	conn, err := net.Dial("tcp", t.Address+":"+strconv.Itoa(t.Port))
